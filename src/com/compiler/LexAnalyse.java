...
public class LexAnalyse {
    ...
    private static boolean isInteger(String word) {
        int i;
        boolean flag = false;
        for (i = 0; i < word.length(); i++) { //读入的文件单词长度
    ...
    private static boolean isID(String word) {
         ...
         * 第二个字母可以使字母、数字或者下划线
         * */
        if (isLetter(temp) || temp == '_') { //字母或者下划线
            for (i = 1; i < word.length(); i++) {
                temp = word.charAt(i);
    ...
    public void analyse(String str, int line) {
        ...
        char temp;
        while (index < length) {
            temp = str.charAt(index); //字符在字符串中的下标
            if (!noteFlag) {
                /*
                 * 标识符
                 * */
                if (isLetter(temp) || temp == '_') {// 判断是不是标志符
                    ...
                    word.id = wordCount;
                    word.line = line;
                    word.value = str.substring(beginIndex, endIndex); //获取开始和结束位置
                    /*
                     * 关键字
                     * */
                    if (Word.isKey(word.value)) { //是关键字
                        word.type = Word.KEY;
                    ...
                    if (index < length && isLetter(temp)) {
                        ...
                        if (index < length && str.charAt(index) == '\'') {
                            ...
                            word.type = Word.CHAR_CONST; //字符常量
                        } else {
                            word = new Word();
                            wordCount++;
                            word.id = wordCount;
                 ...
                 * */
                    ...
                    if (index < length && str.charAt(index) == '=') {
                        ...
                        word.line = line;
                        word.value = str.substring(beginIndex, endIndex);
                        word.type = Word.OPERATOR;  //判断为运算符
                    } else {
                        // endIndex=index;
                        word = new Word();
                        wordCount++;
